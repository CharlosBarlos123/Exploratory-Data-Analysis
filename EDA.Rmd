...
# Dutch Adolesence Heights by Region
...
## Permutation Test with Kolmogorov-Smirov Statistic

I am interested in children aged 10 years or older, where growth was considered "significant" to determine any real changed between regions. I will use a permutation test based on the Kolmogorov–Smirnov (KS) statistic to assess whether the age distributions in the two regions (north and south) are comparable.
...
Here is the observed KS test statistic.
...
Then, I will demonstrate the computation of the p-value and 95% confidence interval for the p-value.
...
Next, I will calculate a 95% bootstrap confidence interval for the difference (north minus south) of the means of the height distributions.
...
I am analyzing daily EUR/USD exchange rate data from February 4 to April 15. Let $R_t = log(Y_t)-log(Y_{t-1})$ denote the log return (i.e., the difference of the log exchange rate) on trading day $t$, where $Y_t$ is the exchange rate on day $t$. In this analysis, I will work with the standardized log returns, that is, the data I analyze will be...

$$
X_t = \frac{R_t-R}{s_R}
$$

Where $R$ is the sample mean and $s_R$ is the sample standard deviation of $R_t$. Note that $t$ increases only on trading days, so weekends and holidays are skipped. I want to assess whether there is evidence of a structural change in the dynamics of the log returns on April 2. To investigate this, I will compare two models. Under the assumption of weak form market efficiency, both models assume a $X_t$ is driven by a white noise process but one model allows for a structural break.

Throughout this analysis, I treat the data as a regular time series and ignore potential irregularities in the actual timing of observations due to weekends and holidays.
...
Using mathematical notation, I will derive the log-likelihood function for Model 1.
...
Here I will write an R function to compute the negative log-likelihood for the Model 1. I use optim() to estimate the parameter $\sigma^2$. As a starting value, I will use the sample variance for $\sigma^2$.
...
Using mathematical notation, I will derive the log-likelihood function log $L(\sigma_1^2, \sigma_2^2)$ for Model 2.
...
Like model 1, I will write an R function to find the negative log-likelihood for model 2.
...
Here, I will use a likelihood ratio (LR) test to test whether introducing the break significantly improves model fit.
Recall,
...
## Simulating to Check Assumptions

I am uncertain whether the regularity conditions required for the likelihood ratio test to be valid are fully satisfied. To address this concern, I decide to use a simulation-based approach to approximate the distribution of the LR statistic under the null hypothesis.

Starting from Model 1, I will use the parameter estimate obtained earlier to simulate 1000 synthetic time series under the null hypothesis (i.e., assuming no structural break).
